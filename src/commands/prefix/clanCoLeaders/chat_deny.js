...
module.exports = {
    structure: {
        name: 'chatdeny',
        description: 'Deny a Role/User in you clan channels.',
        aliases: ['denychat'],
        cooldown: 8000,
    },
    /**
    ...
    run: async (client, message, args) => {
        ...
        if (subcommand === 'user') {
            ...
            message.reply({content: `- <@${targetUser.id}> has been denyed from your clan chat channel.`, flags: [ 4096 ]});
            ...
            message.reply({content: `- <@&${targetRole.id}> has been denyed from you clan chat channel.`, flags: [ 4096 ]});
        } else {
...
async function updateChannelPermissions(clanInfo, target, message) {
    ...
    await clanChatChannel.permissionOverwrites.create(target, {
        SendMessages: false,
